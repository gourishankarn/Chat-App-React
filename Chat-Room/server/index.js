...
io.on("connect", socket => {
  socket.on("join", ({ name, room }, callback) => {
    ...
    /* 
    let priv1 = flag11 >= 0 ? history[flag11].priv : "fail"; */
    console.log(history[flag11]);
    let priv1;
    if (flag11 == -1) {
      priv1 = "fail";
    } else if (flag11 >= 0) {
      priv1 = history[flag11].priv;
    }

    console.log(`private ${priv1}`);
    if (priv1 == "fail") {
      console.log("No history of room available");
    }
    if (priv1 == true) {
      //console.log("entered");
      console.log("passed");
      ...
      console.log("for private room");
      for (var i = flag12; i < history.length; i++) {
        if (history[i].room === user.room) {
          let obj = {
            name: history[i].name,
            mess: history[i].mess
